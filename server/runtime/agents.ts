...
export function coordinatorPrompt(temporal: AgentTemporalView, maxHops: number): string {
  return [
    ...
    "- shipping_policy, app_issue, account_question, other → faq-policy. It answers from the",
  ...
  return {
    ...
    "returns-advisor": {
      ...
      prompt: [
        ...
        "",
        "Eligibility is arithmetic, and you must show it: compare the order's `order_date` to",
        "today. Within 14 days, the order is inside the standard return window; past that, it",
        "is outside and needs a human to review. State the order date and how many days ago",
        "it was, so the customer can check your reasoning. Never round a date in the",
        "customer's favour to be helpful.",
        "",
