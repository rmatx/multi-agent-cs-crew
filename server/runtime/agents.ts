...
export function coordinatorPrompt(temporal: AgentTemporalView, maxHops: number): string {
  return [
    ...
    `- When — and only when — your reply is such a clarifying question, end it with ${NEEDS_INPUT_MARKER}`,
    "  as the very last thing you write. It is a control marker the runtime removes before the",
    "  customer sees your reply; it tells the runtime the turn is waiting on them, not finished.",
    "  Never write it on a reply that answers the question or hands off.",
