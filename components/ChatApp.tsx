 ...
 *
 * Mounted by TWO routes and deliberately only written once: `/` is the customer surface, where
 * demo mode is off unless the deployment or the URL asks for it, and `/final` is the reviewer
 * surface, which passes `forceDemo` and gets the whole thing. A second copy of this file for
 * the second route would have drifted from the first inside a week, and the route that gets
 * demonstrated is the one that would have rotted unnoticed.
 *
...
type ChatAppProps = {
  /**
   * Turn demo mode on at mount, with no query string and no server env var. Set by `/final`.
   *
   * It is a PROP and not another env read because the two surfaces have to be able to disagree:
   * `/` must stay a customer page on the same deployment that serves `/final` to a reviewer.
   * Anything read from the environment would answer both routes at once.
   */
  forceDemo?: boolean;
};

export default function ChatApp({ forceDemo = false }: ChatAppProps) {
  ...
  /*
   * Demo mode. Four ways in, and only the LAST of them fails to work in a built image:
   *
   *   `forceDemo` prop            the `/final` route — run time, and cannot be misconfigured
   ...
   *
   * The last is kept for local `npm run dev` and is deliberately no longer the deployment
  ...
  useEffect(() => {
    ...
    // `forceDemo` is the fourth way in, and the only one that cannot be misconfigured at
    // deploy time — the route itself is the switch.
    if (forceDemo || fromEnv || fromUrl) enableDemo();
    /*
     * `/final` also OPENS the trace panel, which `enableDemo` deliberately does not do. The
     * other three routes in are operator conveniences on a customer page, where an instrument
     * panel that unfolds itself would be an intrusion. `/final` is not a customer page — the
     * trace is a thing the visitor came to look at, and one collapsed by default is one a
     * reviewer never opens.
     */
    if (forceDemo) setTracePanelOpen(true);
  }, [enableDemo, forceDemo]);
